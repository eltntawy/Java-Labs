...
public class FXMLFileSystemTreeController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		treeViewParent.getSelectionModel().selectedItemProperty()
				.addListener(new ChangeListener<TreeItem<File>>() {
					...
					public void changed(
						...
						ObservableList<File> listItems = FXCollections
								.observableArrayList();
						for (TreeItem<File> treeFileItem : newValue
								.getChildren()) {
	...
	public ObservableList<TreeItem<File>> buildChildren(TreeItem<File> treeItems) {
		...
		if (f != null && f.isDirectory()) {
			...
			if (files != null) {
				ObservableList<TreeItem<File>> fileChildern = FXCollections
						.observableArrayList();
				for (File file : files) {
	...
	class ListCellRendrer extends ListCell<File> {
		...
		protected void updateItem(File item, boolean empty) {
			...
			Label l = null;
			Image imageFile = new Image(getClass().getResourceAsStream(
					"file.png"));
			Image imageFolder = new Image(getClass().getResourceAsStream(
					"FilesFolder.png"));
